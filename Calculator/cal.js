  ...
  item.addEventListener('click', (e) => {
    ...
    if (buttonText == 'x') {
      ...
      if (topScreen.value.slice(-1) !== buttonText) {
        // checking last character of bottomScreen value  is not equal current operator
        if (
          topScreen.value.slice(-1) === '-' ||
          topScreen.value.slice(-1) === '+' ||
          topScreen.value.slice(-1) === '/'
        ) {
          // checking last character is equal in any of these operator
    ...
    //  calculating percentage out of 100
      ...
      } else bottomScreen.value = '= ' + screenValue;
    }
